...
\begin{itemize}[leftmargin=2em]
    ...
    \item 审核互动、社区反馈与举报接口；
    \item 通知与消息接口；
    \item 管理端用户管理、资源治理、仪表盘统计、日志管理和模型配置接口。
\end{itemize}
...
检测任务接口 & HTTP & 提供图像检测提交、统一任务状态查询、任务结果列表、模型选择、任务删除和报告下载。 \\ \hline
人工审核接口 & HTTP & 提供审核申请创建、公开任务池、审核任务列表、审核详情、审核提交、进度查询、取消审核和人工审核报告生成等能力。 \\ \hline
审核互动与举报接口 & HTTP & 提供审核任务评论、点赞、评论排序、用户举报和举报处理结果查询。 \\ \hline
通知接口 & HTTP + WebSocket & 提供通知列表、社区反馈、未读数量、已读状态更新、广播通知和实时推送。 \\ \hline
管理端治理接口 & HTTP & 提供管理员登录、用户管理、文件/资源管理、举报审核、日志查询、论文/Review 专项日志、仪表盘统计、模型配置和审核审批。 \\ \hline
\end{longtable}
...
\path{/api/tasks/<task_id>/comprehensive-report/} & GET & 后续统一综合鉴伪报告查看与下载入口，返回图像、论文、Review、人工审核与融合分析章节。 \\ \hline
\path{/api/detection/models/} & GET & 后续用户侧查询当前可用检测模型、模型版本和快速/精准检测模式。 \\ \hline
...
\subsection{人工审核接口}
人工审核接口构成自动检测之后的复核链路。该类接口覆盖审核申请、公开任务池、审核任务查看、审核结果查看、审核提交、审核取消、互动评论和人工审核报告生成。
...
\path{/api/review-tasks/pool/} & GET & 后续公开审核任务池接口，支持按学科、任务类型、紧急程度、风险等级筛选。 \\ \hline
...
\path{/api/review-requests/<review_request_id>/summary/} & GET & 后续获取人工审核汇总结果、分歧说明和阶段性进度。 \\ \hline
\path{/api/review-requests/<review_request_id>/cancel/} & POST & 后续发起人取消或终止未完成审核任务。 \\ \hline
\path{/api/review-requests/<review_request_id>/comments/} & GET/POST & 后续查询或发表评论，评论提交时执行敏感词过滤。 \\ \hline
\path{/api/review-comments/<comment_id>/like/} & POST & 后续点赞或取消点赞审核任务评论。 \\ \hline
\path{/api/review-requests/<review_request_id>/report/} & POST & 后续举报违规审核任务。 \\ \hline
\path{/api/review-comments/<comment_id>/report/} & POST & 后续举报违规评论。 \\ \hline
...
通知接口分为持久化通知接口、社区反馈查询接口和实时推送接口三类。持久化通知负责通知列表和已读状态维护，社区反馈负责将举报处理、评论回复、审核进度、论文/Review 检测反馈和综合报告通知集中呈现，实时推送接口负责检测完成等事件的在线通知。
...
\path{/api/feedback/} & GET & 后续社区反馈聚合入口，按反馈类型返回举报处理、互动回复、审核进展、论文/Review 检测反馈和报告通知。 \\ \hline
\path{/api/feedback/<feedback_id>/} & GET & 后续查看单条反馈详情并返回业务跳转链接。 \\ \hline
...
管理端接口覆盖管理员资料、仪表盘统计、用户管理、文件管理、学术资源管理、举报审核、模型配置、审核审批和日志查询等能力。该类接口支撑当前项目的治理能力，也是后续扩展论文检测日志、Review 检测日志和模型配置管理的入口。
...
\path{/api/admin/resources/} & GET & 后续学术资源管理列表，支持按资源类型、学科标签、状态、风险等级检索图像、论文和 Review。 \\ \hline
\path{/api/admin/resources/<resource_id>/} & GET/PUT/DELETE & 后续查看、修改、状态维护或逻辑删除学术资源。 \\ \hline
\path{/api/admin/reports/} & GET & 后续用户举报处理队列，支持按对象类型、状态、时间筛选。 \\ \hline
\path{/api/admin/reports/<report_id>/handle/} & POST & 后续处理审核任务举报和评论举报，记录处置说明并通知相关用户。 \\ \hline
\path{/api/admin/logs/paper-detection/} & GET & 后续论文检测专项日志查询，支持按任务、用户、时间、状态、模型版本和异常类型筛选。 \\ \hline
\path{/api/admin/logs/review-detection/} & GET & 后续 Review 检测专项日志查询，支持按任务、用户、时间、状态、模型版本和异常类型筛选。 \\ \hline
\path{/api/admin/model-configs/} & GET/POST & 后续模型配置列表与新增接口。 \\ \hline
\path{/api/admin/model-configs/<model_id>/} & GET/PUT/DELETE & 后续模型配置详情、编辑与删除接口。 \\ \hline
\path{/api/admin/model-configs/<model_id>/status/} & POST & 后续启用、停用或灰度模型配置。 \\ \hline
...
\begin{itemize}[leftmargin=2em]
    ...
    \item 增加审核互动与举报接口，使评论、点赞、举报和管理端处置结果形成闭环；
    \item 增加社区反馈聚合接口，使举报处理、评论回复、审核进度、检测反馈和报告通知可在用户端统一展示；
...
\begin{figure}[H]
    \centering
    \resizebox{0.98\textwidth}{!}{\input{figures/tikz-ui-paper-detect-wireframe.tex}}
...
\begin{figure}[H]
    \centering
    \resizebox{0.98\textwidth}{!}{\input{figures/tikz-ui-review-detect-wireframe.tex}}
...
\begin{figure}[H]
    \centering
    \resizebox{0.98\textwidth}{!}{\input{figures/tikz-ui-records-list-wireframe.tex}}
    \caption{用户端检测记录列表页线框图（最小框架）}
\end{figure}

\subsubsection{社区反馈与模型选择页面框架}
\textbf{社区反馈入口：}用户端应在通知中心或独立 \path{/feedback} 页面中集中展示举报处理结果、评论回复、人工审核进度、论文检测反馈、Review 检测反馈和综合报告通知。列表项应包含反馈类型、关联任务、最新状态、是否已读和业务跳转入口。

\textbf{模型展示与选择入口：}检测提交页和任务详情页应展示本次任务使用的模型名称、版本号和检测模式。具备高级权限的用户在提交任务前可在快速检测、精准检测等模式之间选择；前端需依据模型配置接口返回的可用范围禁用无权限或已停用模型。
...
\begin{figure}[H]
    \centering
    \resizebox{0.98\textwidth}{!}{\input{figures/tikz-ui-records-detail-wireframe.tex}}
